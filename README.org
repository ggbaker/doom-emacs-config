...
If the ~lsp-ltex~ package doesn't work then use ~eglot~ instead. Make sure to set eglot as the language server in the init section with ~(lsp +eglot)~.
...
** Undo setup
By default ~undo-tree~ keeps all the undo history files in the .emacs.d directory.
Keep them in a hidden directory relative to the working file instead
#+BEGIN_SRC emacs-lisp :tangle config.el

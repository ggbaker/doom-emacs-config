...
*** Ivy keybindings
By default, pressing =RET= in an ivy-minibuffer on a directory opens the directory listing in a new buffer.
I'd much prefer it to just continue the session in the new directory (by default done with =TAB=).
Switch these two
#+begin_src emacs-lisp :tangle config.el

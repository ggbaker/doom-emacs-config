...
** PDF viewer
When using emacs in a graphical environment, I prefer to view tex output pdfs right in emacs with ~pdf-tools~. I don't want this in terminals, since the ~pdf-tools~ buffer would just display gibberish. The following code accomplishes this. (When using emacs in a terminal emulator within a graphical environment, ~TeX-view~ will call a graphical pdf reader; in a non-graphical environment, it will do nothing.)
#+begin_src emacs-lisp :tangle config.el
(when window-system
